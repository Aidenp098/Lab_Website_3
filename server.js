...
app.get('/team_stats', function(req,res) {
	...
	var query2 = 'select * from football_games where home_score > visitor_score;';
	var query3 = 'select * from football_games where home_score < visitor_score;';
	db.task('get-everything', task => {
		return task.batch([
			task.any(query1),
			task.any(query2),
...
app.get('/player_info/select_player', function(req, res) {
    ...
    .then(data => {
    	res.render('pages/player_info',{
			...
			pass_yrd: data[1][0].passing_yards,
			rush_yrd: data[1][0].rushing_yards,
			rec_yrd: data[1][0].receiving_yards,

			avg_pass: Math.round((data[1][0].passing_yards / data[2][0].count) * 100) / 100,
			avg_rush: Math.round((data[1][0].rushing_yards / data[2][0].count) * 100) / 100,
			avg_rec: Math.round((data[1][0].receiving_yards / data[2][0].count) * 100) / 100
        })
    })
    .catch(error => {
    // display error message in case an error
